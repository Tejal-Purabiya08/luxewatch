...
function ProductDetails() {
  ...
  // Modal states for related products quick view
...
useEffect(() => {
  ...
  // 🛒 ADD TO CART
  // ==========================================
  const addToCart = async (targetProduct, quantityCount) => {
    try {
  ...
  // ❤️ ADD TO WISHLIST
  // ==========================================
  const addToWishlist = async (targetProduct) => {
    try {
      const user = JSON.parse(localStorage.getItem("user"));
  ...
  // Modal logic for Related collection
  const openModal = (targetProduct) => {
  ...
  if (!product) {
    return (
      <div className="allproducts-wrapper d-flex align-items-center justify-content-center" style={{ minHeight: "100vh" }}>
  ...
  return (
    <div className="allproducts-wrapper">
      ...
      <div className="container py-5 mt-5 position-relative" style={{ zIndex: 2 }}>
        ...
        <div className="row align-items-center g-5 modal-content-row mb-5 py-4">
          ...
          <div className="col-md-6 text-center">
            ...
            >
              ...
              <img
                src={product.image}
                alt={product.name}
                className="img-fluid modal-image main-watch-img"
              />
            </motion.div>
          </div>

          {/* Right Column: Product Info */}
          <div className="col-md-6 ps-lg-5 content-column text-start">
            <motion.div
            ...
            >
              ...
              <h2 className="modal-product-title mt-1">{product.name}</h2>
              <div className="price-divider left-align"></div>
              <h3 className="modal-price-tag my-3">
                ₹{product.price?.toLocaleString()}
              </h3>
              <p className="modal-description">
                Experience the pinnacle of fine luxury. Crafted with meticulous
                attention to detail, premium materials, and unparalleled performance 
                for the modern lifestyle.
              </p>

              {/* Quantity Selector */}
              <div className="minimal-qty-selector mb-4 d-flex align-items-center">
                <button 
        ...
        <hr style={{ borderColor: "rgba(201, 169, 98, 0.15)", margin: "60px 0" }} />

        {/* RELATED PRODUCTS SECTION */}
        <div className="text-center mb-5 heading-section">
          <span className="section-badge">Suggestions</span>
      ...
      {/* PREMIUM QUICK VIEW MODAL FOR RELATED PRODUCTS */}
      <AnimatePresence>
          ...
          >
            ...
            >
              ...
              <div className="row align-items-center g-4 modal-content-row">
                ...
                <div className="col-md-6 ps-lg-4 content-column text-start">
                  <span className="modal-brand-badge">{selectedProduct.brand}</span>
                  <h2 className="modal-product-title mt-1">{selectedProduct.name}</h2>
                  <div className="price-divider left-align"></div>
                  <h3 className="modal-price-tag my-3">
                  ...
                  <div className="minimal-qty-selector mb-4">
                    <button onClick={() => setQty((prev) => (prev > 1 ? prev - 1 : 1))} className="qty-btn">
                      <i className="bi bi-dash"></i>
                    </button>
                    <span className="qty-value">{qty}</span>
                    <button onClick={() => setQty((prev) => prev + 1)} className="qty-btn">
                      <i className="bi bi-plus"></i>
                    </button>
                  </div>
      ...
      <style>{`
        ...
        .orb-1 { width: 400px; height: 400px; background: radial-gradient(circle, rgba(201, 169, 98, 0.3) 0%, transparent 70%); top: -100px; right: -100px; }
        .orb-2 { width: 300px; height: 300px; background: radial-gradient(circle, rgba(201, 169, 98, 0.2) 0%, transparent 70%); bottom: 20%; left: -150px; }
        .orb-3 { width: 250px; height: 250px; background: radial-gradient(circle, rgba(201, 169, 98, 0.15) 0%, transparent 70%); top: 50%; right: 10%; }
        ...
        .accent-bar { width: 60px; height: 2px; background: linear-gradient(90deg, transparent, #c9a962, transparent); margin-top: 20px; border-radius: 1px; }
        ...
        .image-holder {
          ...
          overflow: hidden; aspect-ratio: 1 / 1; padding: 20px;
        }
        ...
        .main-watch-img { width: 100%; height: 100%; object-fit: cover; border-radius: 12px; transition: transform 0.6s cubic-bezier(0.16, 1, 0.3, 1); cursor: pointer; position: relative; z-index: 1; }
        ...
        /* Modal & Main Page Details Styles */
